...
void Runtime::start() {
    ...
    LOG_INFO("Runtime started with " << thread_count << " worker threads");
}
...
void Runtime::stop() {
    ...
    LOG_INFO("Runtime stopped - all workers joined");
}
...
void Runtime::worker_task(int id) {
...
#endif
        LOG_INFO("[NUMA] Thread " << id << " -> node " << node << " (cpu " << cpu << ")");
    } else {
        bind_thread_to_cpu(cpu);
        LOG_INFO("[AFFINITY] Thread " << id << " -> cpu " << cpu);
    }
    ...
    constexpr float CHANGE_THRESHOLD = 5.0f;
    ...
    while (running.load()) {
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now_steady - last_bench_time).count() >= 10) {
            ...
            LOG_DEBUG("[BENCH] Node " << node << " Latency: Local=" << current_local_lat 
        ...
        if (cpu < static_cast<int>(cpu_usages.size())) {
            ...
            usage = static_cast<float>(dist(rng));
        }
